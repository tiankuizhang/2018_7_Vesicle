 ...
 * if v0*v2<0, return distance from v0 to boundary
 * assume the the v1,v0,v2,v3 are values of the level set function from left
 * to right
 ******************************************************************************/
__device__ inline
...
// calculate the reintialization step
__global__
...
__global__
void extend_step(double * step, double const * lsf, double const * vx, double const * vy, double const * vz, int rows, int cols, int pges, double dx, double dy, double dz, int num_ele)
{
	int row_idx = blockIdx.x * blockDim.x + threadIdx.x;
	int col_idx = blockIdx.y * blockDim.y + threadIdx.y;
	int pge_idx = blockIdx.z * blockDim.z + threadIdx.z;
