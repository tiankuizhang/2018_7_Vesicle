...
__global__
void re_step_kink_stationary(double * step, double const * lsf, bool const * mask, double const * deltat, double const * xpr, double const * xpl, double const * ypf, double const * ypb, double const * zpu, double const * zpd, int rows, int cols, int pges, double dx, double dy, double dz, int num_ele)
{
	int row_idx = blockIdx.x * blockDim.x + threadIdx.x;
	int col_idx = blockIdx.y * blockDim.y + threadIdx.y;
	int pge_idx = blockIdx.z * blockDim.z + threadIdx.z;
	...
	if( (xpr[ind]<dx && xpl[ind]<dx) || (ypf[ind]<dy && ypb[ind]<dy) || (zpu[ind]<dz && zpd[ind]<dz) ){
		step[ind] = 0;
		return;
	}// if this piont lies within a kink, do not change its value
	...
	if( (r1<dx && r2<dx) || (l1<dx && l2<dx) ){
			step[ind] = 0;
			return;
	}// if near a kind point, do not update its value
...
__global__
void re_step_bo1_stationary(double * step, double const * lsf, bool const * mask, double const * deltat, double const * xpr, double const * xpl, double const * ypf, double const * ypb, double const * zpu, double const * zpd, int rows, int cols, int pges, double dx, double dy, double dz, int num_ele)
{
	int row_idx = blockIdx.x * blockDim.x + threadIdx.x;
	int col_idx = blockIdx.y * blockDim.y + threadIdx.y;
	int pge_idx = blockIdx.z * blockDim.z + threadIdx.z;
	...
	if( xpr[ind]<dx || xpl[ind]<dx || ypf[ind]<dy || ypb[ind]<dy || zpu[ind]<dz || zpd[ind]<dz ){
		step[ind] = 0;
		return;
	}// for a node immediately next to the boundary, do not change its value
...
__global__
void re_step_bo2_stationary(double * step, double const * lsf, bool const * mask, double const * deltat, double const * xpr, double const * xpl, double const * ypf, double const * ypb, double const * zpu, double const * zpd, int rows, int cols, int pges, double dx, double dy, double dz, int num_ele)
{
	int row_idx = blockIdx.x * blockDim.x + threadIdx.x;
	int col_idx = blockIdx.y * blockDim.y + threadIdx.y;
	int pge_idx = blockIdx.z * blockDim.z + threadIdx.z;
	...
	if( r2<dx || l2<dx ){
		step[ind] = 0;
		return;
	}// for a node next to a node immediately next to the boundary, do not update its value
