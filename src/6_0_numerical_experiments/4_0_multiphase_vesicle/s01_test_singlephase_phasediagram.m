...
GridSize = [64,64,64];
[x,y,z,f] = SD.Shape.Ellipsoid(GridSize,0.6,"o");
grid = SD.GD3(x,y,z);
...
ExpectedVolume = InitialVolume * 0.4;
ExpectedAreaDifference = 8.*pi*EquivalentRadius * 1.8;
...
for i = 0:3000
	...
	%tmp = sign(tmp) * min(abs(tmp), 0.001*ExpectedAreaDifference) / 2.;
	...
	if mod(i,20)==0 || i==2
		...
		subplot(2,2,[1,3])
		titlestr = [ sprintf('rd:%.2f,rad:%.2f,gr:(%d,%d,%d)\n', ReducedVolume, CurrentReducedAreaDifference,GridSize(1),GridSize(2),GridSize(3) ) ];
		title(titlestr)
		...
		set(th,'BackgroundColor', 'k', 'EdgeColor', 'k')
		set(gca,'Color','k')
