...
for i = 1:iter
	...
	% now determined the appropriate time step
	mask = abs(map.F)<2*map.GD3.Ds;
	MaxSpeedBend = max(abs(NormalSpeedBend(mask)));
	...
	if mod(i,20)==0 || i==2
		...
		titlestr = [ sprintf(' %5d: %.3f, rd:%.3f, shift:(%1d,%1d,%1d)\n kd:%.1f,ko:%.1f,kl:%.1f,kg:%.1f ', i,time,ReducedVolume,sign(x_shift),sign(y_shift),sign(z_shift), ...
				KappaB,KappaBLo,KappaL,KappaG) ];
		title(titlestr)
		...
		titleStr = [ sprintf('energy:%.3f',ene) ];
		title(titleStr)
