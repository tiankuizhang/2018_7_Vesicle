...
%	 nxpr(current),nxpl(current)]
	...
	diff = xpr - nxpr;max(abs(diff(:)))
	diff = xpl - nxpl;max(abs(diff(:)))
	diff = ypf - nypf;max(abs(diff(:)))
	diff = ypb - nypb;max(abs(diff(:)))
	diff = zpu - nzpu;max(abs(diff(:)))
	diff = zpd - nzpd;max(abs(diff(:)))
